...
import { ButtonGroup, ButtonGroupText } from "@adscrush/ui/components/button-group"
import {
  DropdownMenu,
...
function FilterRemoveButton({ className: _className, icon = <IconX />, ...props }: FilterRemoveButtonProps) {
  const context = useFilterContext()
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-fit min-w-fit">
        {operators.map((op) => (
          ...
          >
            <span>{op.label}</span>
            <IconCheck className={cn("ms-auto text-primary", op.value === operator ? "opacity-100" : "opacity-0")} />
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
