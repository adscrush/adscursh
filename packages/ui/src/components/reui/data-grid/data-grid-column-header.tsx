...
import { getColumnHeaderLabel, useDataGrid } from "@adscrush/ui/components/reui/data-grid/data-grid"
...
function DataGridColumnHeaderInner<TData, TValue>({
  ...
  const headerLabelClassName = cn(
    "inline-flex h-full items-center gap-1.5 text-xs/relaxed font-normal text-secondary-foreground/80 [&_svg]:size-3 [&_svg]:opacity-60",
    className
  )

  const headerButtonClassName = cn(
    "-ms-2 h-6 rounded-md px-2 font-normal text-secondary-foreground/80 hover:bg-secondary hover:text-foreground data-[state=open]:bg-secondary data-[state=open]:text-foreground",
    className
  )
  ...
  const sortIcon =
    canSort &&
    (isSorted === "desc" ? (
      <IconArrowDown className="size-3.25" />
    ) : isSorted === "asc" ? (
      <IconArrowUp className="size-3.25" />
    ) : (
      <IconSelector className="mt-px size-3.25" />
    ))
    ...
    if (canSort) {
      ...
      items.push(
        ...
        >
          ...
          {isSorted === "asc" && <IconCheck className="size-4 text-primary opacity-100!" />}
        </DropdownMenuItem>,
        <DropdownMenuItem
          key="sort-desc"
          onClick={() => {
        ...
        >
          ...
          {isSorted === "desc" && <IconCheck className="size-4 text-primary opacity-100!" />}
        </DropdownMenuItem>
      )
      hasPreviousSection = true
    }
    ...
    if (props.tableLayout?.columnsPinnable && canPin) {
      ...
      items.push(
        <DropdownMenuItem key="pin-left" onClick={() => column.pin(isPinned === "left" ? false : "left")}>
          ...
          {isPinned === "left" && <IconCheck className="size-4 text-primary opacity-100!" />}
        </DropdownMenuItem>,
        <DropdownMenuItem key="pin-right" onClick={() => column.pin(isPinned === "right" ? false : "right")}>
          ...
          {isPinned === "right" && <IconCheck className="size-4 text-primary opacity-100!" />}
        </DropdownMenuItem>
      )
      hasPreviousSection = true
    }
