...
function DataGridTableVirtualStatusRow({
  ...
  return (
    <tr>
      <td colSpan={columnCount} className={cn("py-4 text-center text-sm text-muted-foreground", className)}>
        {children}
      </td>
    </tr>
  )
...
function DataGridTableVirtualBody<TData>({
  ...
  if (isVirtualizationEnabled) {
    if (leadingSpacerHeight > 0) {
      renderedRows.push(
        <DataGridTableVirtualSpacer key="virtual-spacer-start" columnCount={columnCount} height={leadingSpacerHeight} />
      )
    }
