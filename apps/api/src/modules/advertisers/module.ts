...
import { createAdvertiserSchema, updateAdvertiserSchema } from "@adscrush/shared/validators/advertiser.validator"
...
export const advertiserRoutes = new Elysia({ prefix: "/advertisers" })
  ...
  .get("/", async ({ query }) => {
    ...
    if (accountManagerId) conditions.push(eq(advertisers.accountManagerId, accountManagerId))
    const where = conditions.length > 0 ? and(...conditions) : undefined

    const [items, countResult] = await Promise.all([
      db.select().from(advertisers).where(where).limit(limit).offset(offset).orderBy(advertisers.createdAt),
      db.select({ count: sql<number>`count(*)` }).from(advertisers).where(where),
    ])
  ...
  // ── GET /:id ───────────────────────────────────────────────────
  .get("/:id", async ({ params }) => {
    const [advertiser] = await db.select().from(advertisers).where(eq(advertisers.id, params.id)).limit(1)
