...
const Header = () => {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 border-b transition-all duration-300 ${scrolled ? "soft-border bg-background/80 backdrop-blur-xl" : "border-transparent bg-transparent"}`}>
        ...
        <nav className="hidden items-center lg:flex">
          {navLinks.map((link, index) => (
            <div key={link} className="flex items-center">
              ...
              {index < navLinks.length - 1 ? <span className="mx-9 h-5 w-px bg-white/12" /> : null}
            </div>
          ))}
        </nav>

        <div className="flex items-center gap-3">
          <Button asChild variant="hero" className="hidden h-9 px-5 lg:inline-flex">
      ...
      {mobileOpen && (
        <div className="home-glass-surface border-t border-border/10 px-6 py-4 lg:hidden animate-in fade-in slide-in-from-top-4 duration-300">
