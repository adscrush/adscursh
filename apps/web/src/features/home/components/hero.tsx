...
const HeroSection = () => {
  ...
  return (
    <section id="home" className="mx-auto max-w-[1440px] px-3 sm:px-6 lg:px-8">
      <div className="gradient-card relative overflow-hidden rounded-b-[35px] px-5 pb-6 pt-24 sm:px-8 lg:px-16 lg:pt-28">
        ...
        <div data-reveal className={`${sectionClass} relative z-10 mx-auto flex max-w-3xl flex-col items-center text-center`}>
          ...
          <h1 className="mx-auto max-w-[980px] text-balance text-4xl font-medium leading-tight md:text-6xl">
          ...
          <p className="mt-5 max-w-[541px] text-sm text-foreground/70 md:text-base">
            Adscrush gives affiliate teams and performance operators a clear view of attribution, offer health, partner quality, and campaign performance from one dark, focused dashboard.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Button asChild variant="hero" size="xl">
              <Link href="/sign-up">Start with Adscrush</Link>
            </Button>
        ...
        <div data-reveal className={`${sectionClass} relative z-10 mt-10 w-full overflow-visible lg:mt-14`}>
          ...
          <div className="relative -mt-16 mb-[-100px] w-full lg:-mt-16">
            <div className="mx-auto max-w-[1240px] [perspective:1200px]">
              <div className="origin-top [transform:rotateX(10deg)] scale-[0.87] lg:scale-[0.9]">
                  ...
                  <div className="relative mx-auto mt-8 w-full max-w-[1300px] overflow-hidden rounded-t-2xl border border-white/10 bg-[#010205] shadow-[inset_0_0_120px_rgba(255,255,255,0.04),inset_0_0_60px_rgba(255,255,255,0.03),inset_0_0_30px_rgba(255,255,255,0.02)] md:mt-12">
                    <div className="flex h-[380px] bg-[#010205] sm:h-[430px] md:h-[540px]">
                      <aside className="hidden w-48 flex-col md:flex">
                        ...
                        <div className="p-3">
                          <span className="mb-2 block text-[9px] uppercase tracking-wider text-white/40">General</span>
                        ...
                        <div className="flex-1 p-3">
                          <span className="mb-2 block text-[9px] uppercase tracking-wider text-white/40">Top Features</span>
                          <div className="flex flex-col gap-0.5">
                            {[
                              "Offer visibility",
                              "Postback health",
                              ...
                              <div key={item} className="rounded-md px-2 py-1.5 text-[10px] text-white/50 transition-colors hover:bg-white/5 hover:text-white/70">
                                {item}
                              </div>
                            ))}
                          </div>
                      ...
                      <div className="m-2 flex min-w-0 flex-1 flex-col rounded-xl border border-white/[0.06] shadow-[inset_0_0_40px_rgba(255,255,255,0.06),inset_0_0_20px_rgba(255,255,255,0.04)]">
                        <div className="flex items-center justify-between border-b border-white/10 px-3 py-2.5 md:px-4">
                          ...
                          <div className="flex items-center gap-2">
                            <Button variant="glass" className="hidden h-8 rounded-lg border-white/10 bg-white/5 px-2.5 text-[10px] text-white/70 hover:bg-white/10 sm:inline-flex">Invite</Button>
                            <Button variant="hero" className="h-8 rounded-lg px-2.5 text-[10px]">Create report</Button>
                          </div>
                        </div>

                        <div className="px-3 py-3 md:px-4">
                          <h3 className="text-sm font-semibold text-white">Welcome back, team.</h3>
                          <p className="text-[10px] text-white/50">Your clicks, conversions, and payout signals are fresh across the active workspace.</p>
                        </div>

                        <div className="grid grid-cols-2 gap-2 px-3 pb-3 md:grid-cols-4 md:gap-3 md:px-4">
                          {stats.map((card, idx) => (
                            <article key={card.name} className="min-w-0 overflow-hidden rounded-xl border border-white/[0.06] p-3 backdrop-blur-sm shadow-[inset_0_0_40px_rgba(255,255,255,0.06),inset_0_0_20px_rgba(255,255,255,0.04)]">
                              <div className="flex items-center justify-between">
                                <div className="flex min-w-0 items-center gap-2">
                                  <div className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-white/[0.04] md:size-8">
                                    {idx % 4 === 0 ? <Layers className="h-4 w-4 text-purple-400" /> : idx % 4 === 1 ? <CircleDollarSign className="h-4 w-4 text-blue-400" /> : idx % 4 === 2 ? <Gem className="h-4 w-4 text-green-400" /> : <Cpu className="h-4 w-4 text-orange-400" />}
                                  </div>
                                  <span className="truncate text-[11px] font-medium text-white md:text-xs">{card.name}</span>
                                </div>
                                <button className="hidden shrink-0 text-white/30 transition-colors hover:text-white/60 md:block">•••</button>
                              </div>
                              <div className="mt-3 flex min-w-0 flex-col gap-1">
                                <span className="text-[8px] uppercase tracking-wider text-white/40 md:text-[9px]">{card.label}</span>
                                <div className="flex items-baseline gap-1.5">
                                  <span className="whitespace-nowrap text-sm font-semibold tracking-tight text-white md:text-lg">{card.value}</span>
                                </div>
                                <span className={`whitespace-nowrap text-[8px] font-medium md:text-[10px] ${card.up ? "text-emerald-400" : "text-red-400"}`}>{card.change}</span>
                              </div>
                            </article>
                          ))}
                        </div>
                        ...
                        <div className="hidden flex-1 sm:block">
                          <div className="mx-3 mb-3 flex flex-1 flex-col md:mx-4">
                            <div className="mb-2 flex items-center justify-between px-1 py-1.5 md:px-0 md:py-2.5">
                              ...
                              <div className="flex items-center gap-1 text-[9px]">
                                {[
                                  ...
                                  <span key={tab as string} className={`rounded-md px-2 py-1 transition-colors ${active ? "border border-white/[0.06] text-white shadow-[inset_0_0_20px_rgba(255,255,255,0.04)]" : "text-white/50 hover:bg-white/5 hover:text-white/70"}`}>
                                    {tab as string}
                                  </span>
                                ))}
                              </div>
                            ...
                            <div className="flex flex-1 flex-col overflow-hidden rounded-xl border border-white/[0.06] shadow-[inset_0_0_40px_rgba(255,255,255,0.06),inset_0_0_20px_rgba(255,255,255,0.04)]">
                              <div className="grid grid-cols-6 gap-2 border-b border-white/10 px-4 py-1.5 text-[8px] uppercase tracking-wider text-white/40">
                                <span>#</span>
                                <span>Product</span>
                                <span>Category</span>
                                <span>Price</span>
                              ...
                              <div className="flex flex-col">
                                {tableRows.map((row) => (
                                  <div key={row.id} className="grid grid-cols-6 gap-2 border-b border-white/5 px-4 py-2 text-[10px] transition-colors hover:bg-white/[0.02]">
                                    ...
                                    <svg className="h-4 w-12" viewBox="0 0 48 16">
                                      <path
                                        d={row.up ? "M0 12 L8 10 L16 8 L24 9 L32 6 L40 4 L48 2" : "M0 4 L8 6 L16 8 L24 7 L32 10 L40 12 L48 14"}
                                        fill="none"
