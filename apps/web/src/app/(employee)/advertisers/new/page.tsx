...
export default function NewAdvertiserPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data?.success && response.data.data) {
        router.push(`/advertisers/${response.data.data.id}`)
      } else {
        setError(response.data?.error || "Failed to create advertiser")
      }
    } catch (err) {
      setError(
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <PageHeader title="Create Advertiser" description="Loading..." />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Add a new advertiser to your network"
      >
          ...
          <FieldGroup>
            ...
            <Field>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                The advertiser will be prompted to change this password on first
                login
              </p>
            </Field>
