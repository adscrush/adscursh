...
import env from "../env"
import { validateOfferAndAffiliate, selectLandingPage, recordClick } from "../services/click.service.js"
import { z } from "zod"
...
export const clickRoute = new Elysia().get("/c", async ({ query, request, redirect }) => {
  ...
  try {
    ...
    const selected = await selectLandingPage(db, offer, lpId)
    ...
    // Build redirect URL
    let redirectUrlStr: string = redirectBase ?? offer.offerUrl
    ...
    try {
      ...
      return redirect(finalRedirectUrl ?? offer.offerUrl)
    } catch (urlError) {
